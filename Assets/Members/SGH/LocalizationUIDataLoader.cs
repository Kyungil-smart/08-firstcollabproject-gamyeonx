...
public class LocalizationUiData : MonoBehaviour
{
    [Header("Google Sheet Settings")]
    [SerializeField] private string sheetURL = "https://docs.google.com/spreadsheets/d/1GOnAJjcQV4ZlfeWOjQxG4L6o8uYbqgEyCePhL-jTcSw/export?format=tsv&gid=1504793523";
    [SerializeField] private string tableName = "ProjectTable";
    ...
    {
        Debug.Log("구글 시트 ProjectTable 로컬라이제이션 로드 시작...");
        using var request = UnityWebRequest.Get(sheetURL);
        yield return request.SendWebRequest();

        if (request.result != UnityWebRequest.Result.Success)
        {
            Debug.LogError($"ProjectTable 시트 로드 실패: {request.error}");
            yield break;
        }
...
#endif
        Debug.Log("모든 ProjectTable 로컬라이제이션 데이터 매칭 완료!");
    }
}
