...
/*
[Unity 연결 방법]
1. 빈 오브젝트에 TurnGuestExitManager를 추가한다.
2. GameTime과 GuestSpawner를 연결한다.
3. 새 턴 시작 시 ResetTurnState()가 호출되어야 한다.
4. 3분이 되면 손님 퇴장을 시작하고,
   손님이 전부 없어질 때까지 다음 주차로 넘어가지 않는다.
5. 4분까지 남아 있으면 강제 삭제 후 다음 주차로 넘어간다.
*/